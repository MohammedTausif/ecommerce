
export const Menu = [
    //Women
    {
        name: 'Women',
        url: "https://pangaia.com/cdn/shop/files/Hope_Print_Graphic_T-Shirt_Reflect_Blue_Womens-1.png?crop=center&height=1050&v=1741860506&width=1400",
        subMenu: [
            {
                category: 'Trending',
                list: [
                    { name: 'New In', Link: '/new-arrival-women' },
                    { name: 'Spring Lookbook', Link: '/' },
                    { name: 'Tracksuits', Link: '/' },
                    { name: 'The T-shirt Guide', link: '/' }
                ]
            },
            {
                category: 'Categories',
                list: [
                    { name: 'Shop All', Link: '' },
                    { name: 'Hoodies & Sweatshirts', Link: 'collections/womens/hoodies-sweatshirts' },
                    { name: 'Track Pants', Link: 'collections/womens/trackpants' },
                    { name: ' T-Shirts & Tops', Link: '' },
                    { name: 'Shorts', Link: '' },
                    { name: 'Activewear', Link: '' },
                    { name: 'Pants', Link: '' },
                    { name: 'Knitwear', Link: '' },
                    { name: 'Coats & Jackets', Link: '' },
                    { name: 'Pyjamas', Link: '' },
                    { name: 'Footwear', Link: '' },
                    { name: 'Accesories', Link: '' },
                ]
            },
            {
                category: 'Shop By Colour',
                list: [
                    { color: "Black", Link: '' },
                    { color: "White", Link: '' },
                    { color: "Blue", Link: '' },
                    { color: "Pink", Link: '' },
                    { color: "Green", Link: '' },
                    { color: "Grey", Link: '' },
                    { color: "Nuetral", Link: '' },
                    { color: "Red", Link: '' },
                    { color: "Purple", Link: '' },
                ]
            },
            {
                category: 'Shop By Capsule',
                list: [
                    { name: 'DNA', Link: '' },
                    { name: '365 Midweight', Link: '' },
                    { name: 'Heavyweight', linkl: '' },
                    { name: 'Linen', Link: '' }
                ]
            }

        ]
    },

    //Men
    {
        name: 'Men',
        url: "https://pangaia.com/cdn/shop/files/Hope_Print_Graphic_T-Shirt_Off_White_Mens-4_52310e03-f929-4176-a663-2b0778627fa7.png?crop=center&height=1050&v=1741860406&width=1400",
        subMenu: [
            {
                category: 'Trending',
                list: [
                    { name: 'New In', Link: '/new-arrival-men' },
                    { name: 'Spring Lookbook', Link: '/shop-all?filter.p.product_type=hoodies' },
                    { name: 'Tracksuits', Link: '/' },
                    { name: 'The T-shirt Guide', link: '/' }
                ]
            },
            {
                category: 'Categories',
                list: [
                    { name: 'Shop All', Link: '' },
                    { name: 'Hoodies & Sweatshirts', Link: 'collections/mens/hoodies-sweatshirts' },
                    { name: 'Track Pants', Link: 'collections/mens/trackpants' },
                    { name: ' T-Shirts & Tops', Link: 'collections/mens/men-tops' },
                    { name: 'Shorts', Link: 'collections/mens/men-shorts' },
                    { name: 'Pants', Link: 'collections/mens/men-pants' },
                    { name: 'Activewear', Link: 'collections/mens/mens-activewear' },
                    { name: 'Knitwear', Link: 'collections/mens/men-knitwear' },
                    { name: 'Pyjamas', Link: 'collections/mens/mens-loungewear' },
                    { name: 'Coats & Jackets', Link: 'collections/mens/men-jackets-and-outwear' },
                    { name: 'Footwear', Link: 'collections/mens/footwear' },
                    { name: 'Accesories', Link: 'collections/mens/accesories' },
                ]
            },
            {
                category: 'Shop By Colour',
                list: [
                    { color: "Black", Link: '' },
                    { color: "White", Link: '' },
                    { color: "Blue", Link: '' },
                    { color: "Green", Link: '' },
                    { color: "Pink", Link: '' },
                    { color: "Grey", Link: '' },
                    { color: "Nuetral", Link: '' },
                    { color: "Red", Link: '' },
                    { color: "Purple", Link: '' },
                ]
            },
            {
                category: 'Shop By Capsule',
                list: [
                    { name: 'DNA', Link: '/DNA' },
                    { name: '365 Midweight', Link: '' },
                    { name: 'Heavyweight', linkl: '' },
                    { name: 'Linen', Link: '' }
                ]
            },
        ]
    },
    //Kids
    {
        name: 'Kids',
        url: "https://pangaia.com/cdn/shop/files/PANGAIA-Winter-Lookbook-9X16_copy_45.jpg?crop=center&height=768&v=1738578991&width=1024",
        subMenu: [
            {
                category: 'Trending',
                list: [
                    { name: 'New In', Link: '/new-arrival-women' },
                    { name: 'Babywear', Link: '/' },
                    { name: 'Mini-Me', Link: '/' },
                ]
            }, {
                category: 'Categories',
                list: [
                    { name: 'Shop All', Link: '' },
                    { name: 'Hoodies & Sweatshirts', Link: '' },
                    { name: 'Track Pants', Link: '' },
                    { name: 'T - Shirts', Link: '' },
                    { name: 'Playwear', Link: '' },
                    { name: 'Coats & Jackets', Link: '' },
                    { name: 'Footwear', Link: '' }
                ]
            },
            {
                category: 'Shop by Color',
                list: [
                    {color: 'Black'},
                    {color: 'White'},
                    {color: 'Blue'},
                    {color: 'Grey'},
                    {color: 'Beige'},
                    {color: 'Pink'},
                    {color: 'Purple'},
                    {color: 'Green'},
                    {color: 'Red'}
                ]
            }
        ]
    },
    {
        name: 'Our Mission',
        url: "https://pangaia.com/cdn/shop/videos/c/vp/5083542b159e4e71a14b447cfec15d80/5083542b159e4e71a14b447cfec15d80.HD-720p-4.5Mbps-32378110.mp4",
        subMenu: [
            {
                category: 'About Us',
                list: [
                    { name: 'ROGER Lab', Link: '' },
                    { name: 'ROGER Stories', Link: '' },
                    { name: 'People', Link: '' },
                    { name: 'Planet', Link: '' },
                    { name: 'Impact Report 2023', Link: '' },
                    { name: 'Powered by ROGER', Link: '' },
                ], Link: ''
            },
            {
                category: 'Meet our Materials',
                list: [
                    { name: 'MIRUM', Link: '' },
                    { name: 'FLWRDWN™', Link: '' },
                    { name: 'Organic Cotton', Link: '' },
                    { name: 'C-FIBER™', Link: '' },
                    { name: 'FRUTFIBER™', Link: '' },
                    { name: "Grape 'Leather' ", Link: '' },
                    { name: 'Recycled Wool', Link: '' },
                    { name: 'NATIVARegen™', Link: '' },
                    { name: 'ROGettle™ Denim', Link: '' },
                    { name: 'Aloe Linen', Link: '' }
                ]
            },
            {
                category: 'Philanthropy',
                list: [

                    { name: 'ROGER x CORAL GARDENERS', Link: '' },
                    { name: 'ROGER x CHOOSE LOVE', Link: '' },
                    { name: 'Bee The Change', Link: '' },
                    { name: 'Tomorrow Tree Fund', Link: '' },
                ]
            }
        ]
    }
]