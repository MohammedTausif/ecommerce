...
const ThumbnailCard = ({ product, onClick, selectedtype }) => (
    <div key={product.id} className={` w-48 z-10  overflow-hidden shadow-md cursor-pointer transition-border duration-300 ${product.slug? selectedtype === product.slug ? "border-1 border-black " : "border-1 border-transparent " : selectedtype === product.color ? "border-1 border-black": "border-1 border-transparent" } `}
      onClick={onClick} >
